...
def tests(session):
    ...
    try:
        ...
        session.run(
            ...
            "--omit=src/saltext/pagerduty/*",
            "--include=tests/*",
        )
        try:
            session.run("coverage", "report", "--show-missing", "--include=src/saltext/pagerduty/*")
            ...
            #    "--include=src/saltext/pagerduty/*,tests/*"
            # )
        finally:
...
def gen_api_docs(session):
    ...
    session.run(
        ...
        "docs/ref/",
        "src/saltext",
        "src/saltext/pagerduty/config/schemas",
    )
